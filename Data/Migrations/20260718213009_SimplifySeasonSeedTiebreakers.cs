...
{
    /// <inheritdoc />
    public partial class SimplifySeasonSeedTiebreakers : Migration
    {
        /// <inheritdoc />
...
-- (Division standings placement — unrelated to the season-seed tiebreaker rule below.)
base_ranked AS (
    SELECT
        agg.*,
...
-- ── Step 5: season seed ───────────────────────────────────────────────────────
--
-- Strict 4-step rule (per league policy):
--   a) Finished first in division — 1/0 only when FirstPlaceGuaranteed is true;
--      always 0 (no effect) when FirstPlaceGuaranteed is false.
--   b) Total StandingsPoints for the season.
--   c) Season PlusMinus.
--   d) Head-to-head PlusMinus between the tied teams (only meaningful for teams
--      tied within the same division, since teams never play cross-division —
--      cross-division ties naturally fall through with H2HPlusMinus = 0).
--
-- No other criteria participate (Wins/H2HWins deliberately excluded — previous
-- version used them as extra tiebreakers not part of the specified rule).
--
-- Final fallback: DivisionRank ASC (team that finished higher in its own
-- division wins a still-tied cross-division comparison) — used because a-d
-- alone cannot resolve ties between teams that never played each other.
--
SELECT
    ranked.""DivisionRank"",
    ranked.""DivisionSeed"",
...
");
        ...
        {
            // Restore the previous ordering (Wins and H2HWins included as extra tiebreakers).
            migrationBuilder.Sql(@"DROP VIEW IF EXISTS ""Standings"";");
