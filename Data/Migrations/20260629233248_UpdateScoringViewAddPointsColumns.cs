...
{
    /// <inheritdoc />
    public partial class UpdateScoringViewAddPointsColumns : Migration
    {
        /// <inheritdoc />
...
-- Outer query joins Teams twice for display names and surfaces all computed columns.
SELECT
    core.""Id"",
...
FROM (
    ...
    SELECT
        ...
        -- PointsFor: my raw score for normal play; forfeit opponent bonus for one-sided forfeit.
        CASE
            WHEN sc.my  = -1 THEN 0
        ...
        -- PointsAgainst: sized so (PointsFor - PointsAgainst = PlusMinus margin).
        CASE
        ...
        -- Standings points earned for this game row.
        CASE
            WHEN sc.my  = -1                    THEN 0
            WHEN sc.opp = -1                    THEN s.""PointsForWin""
            WHEN sc.my  = s.""PointsToWinGame""  THEN s.""PointsForWin""
            ELSE 0
        END AS ""WinPTS"",

        0 AS ""TiePTS"",  -- individual games cannot tie

        CASE
            WHEN sc.my  = -1                    THEN 0  -- forfeit: ForfeitPoints applies instead
            WHEN sc.opp = s.""PointsToWinGame""  THEN s.""PointsForLoss""
            ELSE 0
        END AS ""LossPTS"",

        CASE
            WHEN sc.my = -1 THEN s.""PointsForNoShow""
            ELSE 0
    ...
    SELECT
        ...
        -- Standings points earned for this match row.
        CASE
            WHEN     mf.i_forfeit AND NOT mf.opp_forfeit  THEN 0
            WHEN NOT mf.i_forfeit AND     mf.opp_forfeit  THEN s.""PointsForWin""
            WHEN     mf.i_forfeit AND     mf.opp_forfeit  THEN 0
            WHEN (COALESCE(sc.my1,0)+COALESCE(sc.my2,0)) >
                 (COALESCE(sc.opp1,0)+COALESCE(sc.opp2,0)) THEN s.""PointsForWin""
            ELSE 0
        END AS ""WinPTS"",
        ...
        CASE
            ...
            WHEN (COALESCE(sc.my1,0)+COALESCE(sc.my2,0)) =
                 (COALESCE(sc.opp1,0)+COALESCE(sc.opp2,0)) THEN s.""PointsForTie""
            ELSE 0
        END AS ""TiePTS"",

        CASE
            WHEN mf.i_forfeit   THEN 0  -- forfeit: ForfeitPoints applies instead
            WHEN mf.opp_forfeit THEN 0
            WHEN (COALESCE(sc.my1,0)+COALESCE(sc.my2,0)) <
                 (COALESCE(sc.opp1,0)+COALESCE(sc.opp2,0)) THEN s.""PointsForLoss""
            ELSE 0
        END AS ""LossPTS"",
